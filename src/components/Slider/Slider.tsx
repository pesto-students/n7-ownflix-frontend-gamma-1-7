...
const Slider: React.FunctionComponent<SliderProps> = props => {
	...
	return (
		<div className="SliderPosterCard">
			{sliderData.error && (
				<div className="SliderPosterCard__not-loaded">Oops, an error occurred.</div>
			)}
			{sliderData.loading ? (
				...
				<h3 className="SliderPosterCard__title">
					{title === 'Recommended Movies' || 'Continue Watching' ? (
						...
						<Link to={type === 'movies' ? `movies/${genre}` : `series/${genre}`}>
							<span>{title}</span>
							<span className="SliderPosterCard__showmore">
			...
			{!sliderData.loading && !error && (
				<div className="SliderPosterCard__poster--wrp">
					<div className="SliderPosterCard__slider--mask left" ref={navigationPrevRef}>
						<ChevronLeftIcon
							className="SliderPosterCard__slider--mask-icon left"
							fontSize="large"
							style={{ color: 'white' }}
						/>
					</div>
					<div className="SliderPosterCard__slider--mask right" ref={navigationNextRef}>
						<ChevronRightIcon
							className="SliderPosterCard__slider--mask-icon right"
							fontSize="large"
							style={{ color: 'white' }}
						/>
					</div>
