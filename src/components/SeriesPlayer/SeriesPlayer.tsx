...
const SeriesPlayer: React.FunctionComponent<ISeriesPlayerProps> = (props) => {
  ...
  const hasAddedinWatchlist =
    watchlist.series.filter(
      (w: any) => w.series._id === props.playerData.data?._id
    ).length > 0;
  ...
  const handleAdd = (event: any) => {
    ...
    dispatch(
      addToWatchlistAsync(
        "/watch-list/",
        {
          entityId: props.playerData.data?._id,
          user: localStorage.getItem("user"),
          entity: "series",
        },
        "series",
        props.playerData.data as Series
      )
    );
  };
  ...
  if (props.playerData.data?.episodes?.length) {
    if ((props.playerData.data?.episodes || [])?.length > episodeNo) {
      const { title, plot, images, videoMain } = props.playerData.data.episodes[
        episodeNo
      ] as any;
      const { genres, yearOfRelease, imdbRating, rated } = props.playerData
        .data as any;
      const thumbNail = images[0].location.cloudFrontUrl;
      const autoVideoUrl = videoMain.destinationLocation.location.cloudFrontUrl;
      const sdVideoUrl =
        autoVideoUrl.split(".").slice(0, -1).join(".") +
        "_Ott_Hls_Ts_Avc_Aac_16x9_480x270p_0.4Mbps_qvbr.m3u8";
      const hdVideoUrl =
        autoVideoUrl.split(".").slice(0, -1).join(".") +
        "_Ott_Hls_Ts_Avc_Aac_16x9_960x540p_3.5Mbps_qvbr.m3u8";
